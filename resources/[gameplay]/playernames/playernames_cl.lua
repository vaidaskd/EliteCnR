    ...
    for _, i in ipairs(GetActivePlayers()) do
        ...
        if i ~= PlayerId() then
            ...
            if distance < 250 and HasEntityClearLosToEntity(PlayerPedId(), ped, 17) then
                -- GAMER_NAME is never shown by playernames: the CnR resource draws its
                -- own colour-coded nametag (markers.lua). Forcing this false removes the
                -- white default-name flicker that appeared on tag (re)creation.
